...
public class register extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String login = (String) request.getParameter("registerLogin");
            String password = (String) request.getParameter("registerPassword");

            if (!bean.register(login, password)) {
                request.setAttribute("erreur", "Vous devez remplir les deux champs.");

                getServletContext().getRequestDispatcher("/index.jsp")
                        .forward(request, response);
            } else {
                getServletContext().getRequestDispatcher("/servlet/log?login=" + login + "&password=" + password)
                        .forward(request, response);
            }
